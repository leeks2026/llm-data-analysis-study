...
completed 주문 기준으로 상품 카테고리별 총 주문 금액 차이와 가장 많이 기여하는 카테고리를 이해하고 싶다고 입력했다.  
초보 데이터 분석자가 먼저 확인할 분석 질문 5개와 필요한 파일, 컬럼 후보를 제안해 달라고 요청했다.

### LLM 답변 요약
LLM은 다음과 같은 분석 질문 후보를 제안했다.

1. 상품 카테고리별 completed 주문 금액 비교
2. 상품 카테고리별 판매 수량 비교
3. 월별 completed 주문 금액 변화 확인
4. 결제수단별 completed 주문 건수와 주문 금액 비교
5. 고객 연령대별 completed 주문 금액 비교

### 실제 반영 여부
상품 카테고리별 completed 주문 금액 비교를 현재 분석 질문으로 사용했다.

### 사람이 검증한 항목
- 필요한 파일이 실제로 존재하는지 확인했다.
- 필요한 컬럼이 실제 데이터에 있는지 확인했다.
...
### 사람이 보완한 내용
LLM 제안은 그대로 사용하되, 실제 분석에서는 주문 금액 계산식을 quantity × unit_price로 명확히 하기로 했다.  
또한 결과 해석 시 특정 카테고리가 왜 많이 팔렸는지는 단정하지 않기로 했다.

### 남은 확인 사항
- completed 주문만 정확히 필터링되는지 확인해야 한다.
- 세 파일을 병합했을 때 누락되는 데이터가 없는지 확인해야 한다.
- quantity와 unit_price에 결측치나 이상치가 있는지 확인해야 한다.
- 카테고리별 총 주문 금액 외에 판매 수량과 주문 건수도 함께 확인할 필요가 있다.
